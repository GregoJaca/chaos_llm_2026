    ...
    To evaluate SIC, we generated an ensemble of trajectories from close starting points. We begin with a single prompt, tokenize it, and retrieve its initial embedding tensor $\mathbf{E}_0 \in \mathbb{R}^{N_p \times d}$, where $N_p$ is the prompt length and $d$ is the model's hidden dimension. 

    We generate a set of $N_{\text{pert}}$ perturbed initial conditions $\{\mathbf{E}_i\}_{i=1}^{N_{\text{pert}}}$ located at the vertices of a regular simplex of radius $r$ centered at $\mathbf{E}_0$ within a randomly selected $N_{\text{pert}}$-dimensional subspace. This ensures that all perturbations have an identical Euclidean norm of $r$, and are mutually equidistant, avoiding bias in the sensitivity analysis.
    ...
    We evaluate the spectral norm of the softmax Jacobian for a given row $\mathbf{p}$: $\mathbf{J}_{\text{sm}}(\mathbf{p}) = \text{diag}(\mathbf{p}) - \mathbf{p} \mathbf{p}^T$. For any unit vector $\mathbf{v}$, the Rayleigh quotient $\mathbf{v}^T \mathbf{J}_{\text{sm}}(\mathbf{p}) \mathbf{v} = \sum p_i v_i^2 - (\sum p_i v_i)^2$ defines the statistical variance of $\mathbf{v}$'s coordinates under the distribution $\mathbf{p}$. By Popoviciu's inequality, the variance of any bounded variable is at most $\frac{1}{4}(v_{\max} - v_{\min})^2$. Because $\mathbf{v}$ is bounded to the ($L_2$) unit hypersphere, $v_{\max}^2 + v_{\min}^2 \le 1$, which bounds $(v_{\max} - v_{\min})^2 \le 2$. Thus, we obtain a strict, state-independent universal bound: $\|\mathbf{J}_{\text{sm}}(\mathbf{A})\|_2 \le 0.5$. This was recently proven more generally \cite{nair2025softmax12lipschitztightbound}.
    ...
    \subsubsection{Finite size perturbation} \label{subsubsec:finite_size}
    ...
    We analyzed the dynamical effect of the different components of the Transformer architecture: Attention, MLP, normalization, and residual connections. We showed how perturbations propagate across these operators. The combination of MLP and Attention, which amplify perturbations, with normalization, which keeps the system bounded, and residual connections, which preserve information, produces a balance between divergence and confinement analogous to chaotic attractors. Attention introduces nonlinear coupling between tokens that propagates information and perturbations during autoregressive generation. We derived a closed-form Mean-Field Theory (MFT) formula predicting the expected isotropic stretching of the MLP block, showing strong agreement with empirical results.
    ...
    Future work could systematically explore how chaotic characteristics scale with model size, architecture, and training regime. Comparing encoder-only, decoder-only, and encoder-decoder architectures could illuminate how bidirectional vs. causal attention affects dynamical stability. Moreover, investigating the influence of hyperparameters such as context length, normalization type, and activation function could provide insighths into different dynamical regimes in which the models might operate.
