    ...
    For the MLP blocks, we analyze the token-wise Jacobian $J(x) = \frac{\partial f(x)}{\partial x} \in \mathbb{R}^{d_{model} \times d_{model}}$ of each isolated component:
    We use vectorized reverse-mode automatic differentiation:
    \begin{equation}
        J_{\text{chunk}}(x) = \texttt{vmap}(\texttt{jacrev}(\text{MLP}))(x)
    \end{equation}
    We immediately compute the spectral norm $\|J(x)\|_2 = \sigma_{\max}(J(x))$ and the expected isotropic expansion $\bar{\lambda} = \frac{1}{d_{model}} \|J(x)\|_F^2$. The static weight metrics were computed directly from the model's parameters, while the dynamic activation densities were obtained by manually fetching the intermediate pre and post activation hidden states. 
    ...
    \begin{equation}
        \mathbf{J}_{\text{Attn}} = \frac{\partial Y}{\partial X} \in \mathbb{R}^{(N d_{model}) \times (N d_{model})}
    \end{equation}
    Materializing this dense matrix requires $\mathcal{O}(N^2 d_{model}^2)$ space, which is impossible for long sequences. Therefore, we compute the global spectral norm $\|\mathbf{J}_{\text{Attn}}\|_2$ using a matrix-free **Power Iteration**. 

    Starting with a random unit vector $v_0 \in \mathbb{R}^{N \times d_{model}}$, $\|v_0\|_F = 1$, we iteratively refine the dominant singular direction without constructing the full Jacobian matrix. At each step $k$, we compute the pushforward (JVP) and pullback (VJP) directly on the execution graph:
    \begin{align}
        u_k &= \mathbf{J}_{\text{Attn}} v_k = \texttt{jvp}(\text{Attn}, (X,), (v_k,))_1 \\
        w_k &= \mathbf{J}_{\text{Attn}}^T u_k = \texttt{vjp}(\text{Attn}, X)(u_k)_1
    \end{align}
    The global spectral norm is then estimated via the Rayleigh quotient $\sigma_{\max}(\mathbf{J}_{\text{Attn}}) \approx \sqrt{\frac{\langle w_k, v_k \rangle}{\langle v_k, v_k \rangle}}$, updating the next direction as $v_{k+1} = w_k / \|w_k\|_F$. We run this iteration for $K = 50$ steps. 
    ...
    \end{equation}
    where $W_{gate}, W_{up} \in \mathbb{R}^{H \times d_{model}}$ and $W_{down} \in \mathbb{R}^{d_{model} \times H}$ are the learned weight matrices, $H$ is the intermediate feed-forward dimension, SiLU is the Sigmoid Linear Unit activation function, and $\odot$ the Hadamard product. 

    To quantify the propagation of a local perturbation $\delta x$, we differentiate $\text{MLP}(x)$ with respect to $x$. Applying the chain rule yields the Jacobian $J(x) \in \mathbb{R}^{d_{model} \times d_{model}}$:
    \begin{equation}
    ...
    To quantify the average phase-space expansion induced by the MLP block, we analyze the mean squared singular value ($\overline{\lambda}$) of its local Jacobian ($J$). For a $MLP(x) = W_2 \phi(W_1 x)$
    ...
    Let $X \in \mathbb{R}^{N \times D}$ be the input sequence of $N$ tokens. We analyze how a perturbation $\delta X$ in the input sequence propagates through the self-attention mechanism, which computes\footnote{For notational simplicity, derivations are shown for a single attention head. The total output is the sum over $H$ heads ($Y = \sum_h A^{(h)} X W_V^{(h)} W_O^{(h)}$).}:
    $$ Y = A X W_V W_O $$
    ...
    We evaluate the differential $\delta Y$ with respect to a perturbation $\delta X$. Applying the product rule yields:
    \begin{equation}
    ...
    \end{equation}
    Because the softmax function ensures $A$ is row-stochastic ($A_{i,j} \ge 0$ and $\sum_j A_{i,j} = 1$), applying the triangle inequality yields:
    ...
    This inequality establishes that $A$ computes a convex mixture and cannot autonomously amplify the perturbation norm. The expansion of this term is constrained by the constant projection matrices, which we define as the static \textbf{Mixing Weight Norm}.
    ...
    The differential of the softmax matrix $A$ is determined by its Jacobian, denoted here as $J_{sm}(A)$:
    \begin{equation}
        \delta A = A \odot \left( \delta Z - (A \odot \delta Z)\mathbf{1}\mathbf{1}^T \right) = J_{sm}(A) \delta Z
    \end{equation}
    We substitute $\delta Z$ into $\delta A$, and then $\delta A$ into the routing term $\delta Y_{\text{route}} = (\delta A) X W_V W_O^T$. 
    ...
    The nonlinear term depends on $J_{sm}(A)$. For a given row $a_i$, the Jacobian of the softmax function is $J_{sm}(a_i) = \text{diag}(a_i) - a_i a_i^T$. If a token attends to a single position with absolute confidence, $a_i$ becomes a one-hot vector, which makes, $J_{sm}(a_i) = \mathbf{0}$, causing $\delta A = \delta Y_{\text{route}}  = \mathbf{0}$.

    To understand if this happends, we compute the \textbf{Attention Entropy} across all heads and tokens, normalized by the causal maximum theoretical entropy $H_{\max}^{\text{causal}}(N) = \frac{1}{N} \sum_{i=1}^{N} \log_2(i)$ to account for the expanding context window:
    \begin{equation}
        \text{Entropy Ratio} = \frac{\frac{1}{H \cdot N} \sum_{h,i} \left( - \sum_{j=1}^{i} A_{i,j}^{(h)} \log_2 A_{i,j}^{(h)} \right)}{H_{\max}^{\text{causal}}(N)}
    \end{equation}
    Our measurements show that the entropy ratio fluctuates between $0.4$ and $0.8$ across all layers. Because the ratio remains positive, the attention distribution does not one-hot saturate. Consequently, $\delta A$  and $\delta Y_{\text{route}}$ do not vanish.

    \paragraph{Global Sequence Jacobian and Numerical Measurement}
    Attention operates as a discrete delay-differential system. Because of the causal mask, a perturbation injected into an early token ($\delta x_k$) is encoded in the key and value caches and re-injected into the outputs of all subsequent tokens in the context window($\delta y_t$ for $t \ge k$). 
    ...
    \begin{figure}[H] 
        ...
        \begin{subfigure}[b]{0.48\linewidth}
            ...
            \caption{Phi-4-mini-instruct}
        \end{subfigure}
        \caption{Static weights norms $\left\|W_Q W_K^T\right\|_2$ and $\left\|W_V W_O^T\right\|_2$}
        \label{fig:attn_static_weights}
    \end{figure}

    Similarly to the previous section, we measure the Routing and Mixing static weights norms $\left\|W_Q W_K^T\right\|_2$ and $\left\|W_V W_O^T\right\|_2$. The results show that these are larger than 1, indicating a capacity for perturbation expansion as can be seen in Fig. \ref{fig:attn_static_weights}. $\|\mathbf{J}_{\text{Attn}}\|_2$ is consistently positive, and its magnitude can be seen to scale with $\sqrt{N}$ \footnote{}
    ...
    For an input hidden state $x \in \mathbb{R}^D$, Root Mean Square Normalization (RMSNorm) computes $y = \frac{x}{\text{RMS}(x)}$, where $\text{RMS}(x) = \frac{1}{\sqrt{D}} \|x\|_2$. The Jacobian of this transformation with respect to a perturbation $\delta x$ is:
    ...
    The eigenvalues $\lambda_i$ of the sub-block Jacobian $J_F$ are shifted to $1 + \lambda_i$. However, eigenvalues do not govern the evolution of a perturbation after a single application of the operator. For a single step, the singular values dictate it.

    $$\|(I + J_F)\delta x\|_2^2 = \|\delta x + J_F \delta x\|_2^2$$
    $$\|J_{res} \delta x\|_2^2 = \|\delta x\|_2^2 + \|J_F \delta x\|_2^2 + 2 \delta x^T (J_F \delta x)$$

    We cannot offer a simpler relationship for the singular values. The residual connection makes models more stable, and preserves information. It will preserve the radial perturbations that the normalization layer's projection matrix $P_{\perp}$ eliminated. In the next section we explore the effect of the complete layer on a perturbation.
    ...
    By interpreting autoregressive generation as a high-dimensional discrete-time dynamical system and analyzing trajectories in hidden-state spacee, we investigated whether the evolution of representations within Transformer-based LLMs exhibit signatures of deterministic chaos.
    
    By analyzing trajectories of hidden states we investigated whether the evolution of representations within an autoregressive Transformer can exhibit behaviors analogous to deterministic chaos.
    ...
    We looked for explanation for this behavior by analyzing the dynamical effect of the different components of the Transformer architecture: Attention, MLP, normalization, and residual connections. We showed how perturbations propagate across these operators. The combination of MLP and Attention, which amplify the perturbations, with normalization and residual connections, which preserve information and keep the system bounded produces a balance between divergence and confinement analogous to chaotic attractors. Attention introduces non-linear coupling between tokens which propagates infromation and perturbations during autoregressive information.
    ...
    we compute the norm of each (row) column vector for $(W_{\text{gate}}$, $W_{\text{up}}$) $W_{\text{down}}$. We compute the Coefficient of Variation (CV): $\text{CV} = \frac{\sigma}{\mu}$ (Standard Deviation divided by Mean). If $\text{CV} \ll 1$, the norms are highly uniform. 
