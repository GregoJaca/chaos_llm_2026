    ...
    We model the autoregressive operation and depth-wise representation evolution of a Large Language Model as a \textbf{high-dimensional, discrete-time dynamical system}. We analyze this system through two distinct trajectories:

    \begin{enumerate}
        \item \textbf{Vertical Trajectory (Depth Evolution):} The state vector is the hidden representation $\mathbf{x}_\ell \in \mathbb{R}^d$ of a token, which evolves across layers $\ell = 1 \dots L$ as $\mathbf{x}_{\ell+1} = \mathbf{x}_\ell + \mathbf{f}_\ell(\mathbf{x}_\ell)$. Since each layer has unique weights, the mapping $\mathbf{f}_\ell$ varies with depth, making the vertical system non-autonomous.
    ...
    From this perspective, the self-attention mechanismserves as a state-dependent nonlinear coupling between different components of this trajectory, analogous to the inter-variable coupling observed in classical chaotic systems. The combination of transformer operators mimics the stretching and folding actions of classical chaotic systems (summarized in Table \ref{tab:analogy}). The Attention and MLP blocks act as stretching mechanisms, amplifying local perturbations when the spectral norm of their Jacobians exceeds $1$. The normalization layers (LayerNorm, RMSNorm) act as folding mechanisms, constraining the state space by projecting hidden states onto a bounded manifold.
    ...
    Our results provide consistent evidence for the possibility of such behavior. Across multiple distance metrics—capturing fundamentally different geometric notions such as alignment, extremal deviation, temporal warping, and global self-similarity—and across different representational spaces, we observed qualitatively robust patterns of trajectory divergence, recurrence, and boundedness.

    We analyzed the dynamical effect of the different components of the Transformer architecture: Attention, MLP, normalization, and residual connections. We showed how perturbations propagate across these operators. The combination of MLP and Attention, which amplify perturbations, with normalization and residual connections, which preserve information and keep the system bounded, produces a balance between divergence and confinement analogous to chaotic attractors. Attention introduces nonlinear coupling between tokens that propagates information and perturbations during autoregressive generation.
