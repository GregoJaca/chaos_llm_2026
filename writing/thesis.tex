    ...
    The capacity for state-dependent perturbation growth arises from the first term. The differential of the causal pre-softmax logits is:
    \begin{equation}
    ...
    \end{equation}
    Our measurements show that the entropy ratio fluctuates between $0.4$ and $0.8$ across all layers. Because the ratio remains positive, the attention distribution does not one-hot saturate. Consequently, $\delta \mathbf{A}$ and $\delta \mathbf{Y}_{\text{route}}$ do not vanish.
    ...
    This study explored the internal dynamics of a Large Language Model (LLM) through the lens of nonlinear dynamical systems. 

    By interpreting autoregressive generation as a high-dimensional discrete-time dynamical system and analyzing trajectories in hidden state space, we investigated whether the evolution of representations within Transformer-based LLMs exhibits signatures of deterministic chaos.

    

    We analyzed the dynamical effect of the different components of the Transformer architecture: Attention, MLP, normalization, and residual connections. We showed how perturbations propagate across these operators. The combination of MLP and Attention, which amplify perturbations, with normalization, which keeps the system bounded, and residual connections, which preserve information, produces a balance between divergence and confinement analogous to chaotic attractors. Attention introduces nonlinear coupling between tokens that propagates information and perturbations during autoregressive generation. We also derived a closed-form Mean-Field Theory (MFT) formula predicting the expected isotropic stretching of the MLP block, showing strong agreement with empirical results.
