...
\usepackage{pgfpages}
\pgfpagesuselayout{resize to}[physical paper width=13.333in, physical paper height=7.5in]
\usetheme{metropolis}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{tabularx}
...
\begin{frame}{Dimensionality \& Jacobian Analysis}
  ...
  \begin{itemize}
    \item Derived from correlation integral $C(\epsilon) \sim \epsilon^{D_2}$:
      ...
      \end{equation*}
    \item Allows one to distinguish between deterministic chaos and random noise
  \end{itemize}

  \textbf{Linearized Perturbation Propagation:}
  An input perturbation $\bm{\delta}_0$, propagates depending on the local Jacobian matrix $\mathbf{J}_f(\mathbf{x}) = \frac{\partial f(\mathbf{x})}{\partial \mathbf{x}}$:
  \begin{equation*}
...
\begin{frame}{Attention Static Weights}
  \begin{columns}
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}
        \item Norms are consistently $> 1$ at all layers.
...
\begin{frame}{Attention Jacobian}
  ...
  \end{equation*}
  \begin{itemize}
    \item Softmax row-stochasticity ($\sum_j A_{i,j}=1$) ensures linear term cannot autonomously amplify the perturbation.
  \end{itemize}
...
\begin{frame}{Normalization and Residual Connections}
  ...
  \begin{itemize}
    ...
    \item Preserves the radial components eliminated by $\mathbf{P}_{\perp}$.
...
\begin{frame}{Scaling and FP16 Precision Limits}
  \begin{columns}
    \begin{column}{0.30\textwidth}
      ...
      \end{equation*}
      \begin{itemize}
        \item Exponents close to 1 indicate linearized regime.
        \item Total layer Jacobian most active in the first and last layers
    ...
    \begin{column}{0.68\textwidth}
      \centering
      \includegraphics[width=0.5\textwidth]{plots_new/deepseek/perturbations/single_token_perturb_all_All_Prompts_Aggregated_aggregated_scaling_rainbow_median_xscale-log_yscale-log.png} \\
      \includegraphics[width=0.5\textwidth]{plots_new/deepseek/perturbations/single_token_perturb_all_All_Prompts_Aggregated_aggregated_extracted_jacobians_together_harmonic_xscale-linear_yscale-log.png}
      \\[0.1cm]
